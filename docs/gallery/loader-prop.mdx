...
```bash
npm install react-beyond @react-beyond/loaderprop
```
...
- `id` - The id of the HOC. Defaults to `"loaderProp"`.
...
import { deepClsxProp } from "@react-beyond/loaderprop"

const el = (
  <Beyond hocs={[deepLoaderProp()]}>
    <App />
  </Beyond>
)
```
